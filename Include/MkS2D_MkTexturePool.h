...
public:
	...
	// group을 주어 묶음으로 해제 가능
	bool LoadBitmapTexture(const MkPathName& filePath, unsigned int group = 0);

	// bitmap texture 참조를 texture에 담아 반환
	// 이전 생성된 객체가 없으면 생성 후 반환
	void GetBitmapTexture(const MkPathName& filePath, MkBaseTexturePtr& texture, unsigned int group = 0);
