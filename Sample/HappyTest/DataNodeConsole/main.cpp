...
public:
	...
	static void GetInfo(const MkHashStr& key, MkStr& resultMsg)
	{
		...
		if (type != ePDT_Undefined)
		{
			...
			{
				resultMsg += MkStr(dataSize) + L" 크기의 배열 :" + MkStr::CRLF;
...
public:
	...
	virtual bool ExcuteMsg(const MkStr& command, const MkArray<MkStr>& argument, MkStr& resultMsg) const
	{
		...
		if ((gTargetNode != NULL) && (command == L"정보") && (argument.GetSize() == 0))
		{
			...
			resultMsg = MkStr(gTargetNode->GetUnitCount()) + L"개의 유닛 존재" + MkStr::CRLF;

			MK_INDEXING_LOOP(keyList, i)
			{
				resultMsg += L"  - " + keyList[i].GetString() + MkStr::CRLF;
			}

			keyList.Clear();
			gTargetNode->GetChildNodeList(keyList);
			resultMsg += MkStr(gTargetNode->GetChildNodeCount()) + L"개의 자식 노드 존재" + MkStr::CRLF;
...
public:
	virtual bool SetUp(MkDataNode& sharingNode)
	{
		...
		MK_DEV_PANEL.MsgToFreeboard(2, L"  - 로드 (file path) : 해당 파일 로드");
		MK_DEV_PANEL.MsgToFreeboard(3, L"  - 정보 : unit과 children의 key 출력");
		MK_DEV_PANEL.MsgToFreeboard(4, L"  - 값 (unit key) : 해당 unit의 값 출력");
		MK_DEV_PANEL.MsgToFreeboard(5, L"  - 이동 (node key or \"..\") : 해당 자식 node, 혹은 부모 node로 이동");
		MK_DEV_PANEL.MsgToFreeboard(6, L"  - 저장 : 해당 node를 text로 저장");
		return true;
	}
