...
bool MkRenderToTexture::SetUp(const MkUInt2& size, eFormat texFormat, int targetIndex)
{
	MK_CHECK((size.x > 0) && (size.y > 0), L"render to texture의 size는 0보다 커야 함")
		return false;

	m_Size = size;
	m_HasAlphaChannel = (texFormat == eRGBA);
	m_BlendType = (m_HasAlphaChannel) ? eS2D_MBT_AlphaChannel : eS2D_MBT_Opaque;

	m_RenderToTextureFormat = texFormat;
	...
	if (MkBaseTexture::SetUp(m_Size, 1, D3DUSAGE_RENDERTARGET, format))
	{
		LPDIRECT3DDEVICE9 device = MK_DISPLAY_MGR.GetDevice();
		D3DFORMAT depthStencilFormat = MK_DISPLAY_MGR.GetDepthStencilFormat();

		if (device->CreateDepthStencilSurface(m_Size.x, m_Size.y, depthStencilFormat, D3DMULTISAMPLE_NONE, 0, TRUE, &m_DepthBuffer, NULL) == D3D_OK)
		{
			_ClearTarget(device);
			return true;
		}
...
{
	if ((m_OldRenderTarget != NULL) && (m_OldDepthBuffer != NULL))
	{
		LPDIRECT3DDEVICE9 device = MK_DISPLAY_MGR.GetDevice();
