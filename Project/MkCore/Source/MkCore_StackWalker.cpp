...
void MkStackWalker::CrashOperation(HANDLE threadID, const CONTEXT *context)
{
	MkStr deadMsg;
	deadMsg.Reserve(512);
	deadMsg += L"//---------------------------------------------------//\n";
	deadMsg += L"// Application Dead!!!\n";
	deadMsg += L"//---------------------------------------------------//";
	MK_LOG_MGR.CreateCrashPage(deadMsg);

	ShowCallstack(threadID, context);

	MK_LOG_MGR.SendCrashLogTo();
}

void MkStackWalker::OnOutput(LPCSTR szText)
{
	std::string tmp = szText;
	MkStr buffer;
	buffer.ImportMultiByteString(szText);
	MK_LOG_MGR.MsgToCrashPage(L"<StackWalker> " + buffer);
}

LONG WINAPI ExpFilter(EXCEPTION_POINTERS* pExp, DWORD dwExpCode)
{ 
	MkStackWalker sw;
	sw.CrashOperation(GetCurrentThread(), pExp->ContextRecord);
